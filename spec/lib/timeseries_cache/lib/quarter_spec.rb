...
describe TimeseriesCache::Quarter do
  ...
  subject { TimeseriesCache::Quarter.new(key: key, redis: redis) }

  describe ".fetch_range" do
    context "with no values set" do
      it "yields and sets the value the correct number of times" do
        expect(subject).to receive(:set?).exactly(5).times.and_return(false)
        expect(subject).to receive(:set).exactly(5).times.and_call_original
    ...
    context "with all values set" do
      it "does not yield and gets the value from redis the correct number of times" do
        expect(subject).to receive(:set?).exactly(5).times.and_return(true)
        expect(subject).to receive(:get).exactly(5).times.and_call_original
